...
public class SimpleCompletionProposal
		implements ICompletionProposal, ICompletionProposalExtension6, ICompletionProposalExtension7 {
	...
	public SimpleCompletionProposal(ProposalProviderSupport completion, PluginContextProvider provider,
			IDocument document, int offset, ProposalProvider proposalProvider, ProposalInfoProvider proposalInfo) {
		this.offset = offset;
	...
	public void apply(IDocument document) {
		...
		try {
			...
			EclipseUtil.logError("Not able to replace by proposal:" + proposalProvider + ", zero offset:" + zeroOffset
					+ ", textBefore:" + textBefore, e, provider);
		}
	 ...
	 * Can be overriden
	 * 
	 * @return <code>false</code> when completion shall be added as defined. if
	 *         <code>true</code> a <code>\n</code> will be added to end
	 *         automatically
	 */
	...
	protected String createIndentionString(IDocument document, int zeroOffset, int offsetBefore) {
		...
		while (offsetBefore > 0) {
			...
			try {
				...
				EclipseUtil.logError("Not able to create indention string for proposal:" + proposalProvider
						+ ", zero offset:" + zeroOffset + ", textBefore:" + textBefore, e, provider);
				return "";
			}
			if (b == '\n' || b == '\r') {
				break;
